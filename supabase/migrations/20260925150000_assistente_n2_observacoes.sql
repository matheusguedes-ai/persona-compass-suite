-- #305 — Assistente, NÍVEL 2: o mentor pode registrar observações sobre um aluno, que a assistente
-- considera nas conversas com ele.
--
-- O FLUXO É DE MÃO ÚNICA, do mentor para a assistente:
--   (a) o ALUNO NUNCA LÊ estas linhas. Nenhuma policy abre leitura para ele — e a de leitura da
--       equipe ainda exclui, de propósito, a observação sobre o PRÓPRIO login de quem consulta (um
--       colaborador que também é aluno da conta não lê o que escreveram sobre ele);
--   (b)/(c) quem garante que a assistente não cita nem atribui ao mentor é o texto de orientação
--       (`src/lib/assistente/instrucoes.server.ts`); aqui só se garante quem lê e quem escreve;
--   o mentor continua SEM caminho nenhum para ler as conversas do aluno — nada daqui toca
--   `assistente_conversas`/`assistente_mensagens`.
--
-- Por que não o campo `people.notes` ("Observações" da ficha): a policy `people_self_read` deixa o
-- aluno ler a própria linha de `people`, com `notes` junto. Aquilo não é lugar de nada que ele não
-- possa ver.
--
-- Dono (regra 2): `conta_id` = a conta do cadastro (`people.mentor_id`), preenchida pelo banco — o
-- servidor não escolhe. `autor_id` = quem escreveu.
--
-- Aponta para `people`, então entra em `previa_fusao`/`fundir_pessoas` (redefinidas abaixo, com uma
-- linha a mais cada; o resto é idêntico ao que está no ar).
--
-- Aditiva (regra 5): tabela nova; as duas funções só ganham uma tabela a tratar, com a mesma
-- assinatura e o mesmo retorno — o código publicado continua funcionando igual.

create table public.assistente_observacoes (
...
create index assistente_observacoes_da_pessoa on public.assistente_observacoes (person_id, criada_em);

comment on table public.assistente_observacoes is
  '#305 — observações do mentor sobre um aluno, para a assistente considerar. O aluno NUNCA lê.';

-- A conta é SEMPRE a do cadastro, e o autor é quem está logado — nada disso vem do cliente.
create or replace function public.assistente_observacao_dono()
...
begin
  ...
  if new.conta_id is null then
    raise exception 'assistente: cadastro não encontrado' using errcode = '23503';
  end if;
...
create trigger assistente_observacoes_dono
  before insert or update on public.assistente_observacoes
  for each row execute function public.assistente_observacao_dono();

alter table public.assistente_observacoes enable row level security;

-- Quem lê e escreve: a equipe da conta que ENXERGA aquele cadastro (dono, colaborador, e o mentor
-- convidado só nos grupos que acompanha — `can_see_person`, a mesma regra da ficha da pessoa).
-- Nunca sobre o próprio login.
create policy aobs_equipe on public.assistente_observacoes
  for all to authenticated
  using (
    conta_id = public.acting_account()
    and public.can_see_person(person_id)
    and not exists (select 1 from public.people p where p.id = person_id and p.user_id = auth.uid())
  )
  with check (
    conta_id = public.acting_account()
    and public.can_see_person(person_id)
...
BEGIN
  ...
  v_mover := jsonb_build_object(
    ...
    'assistente_observacoes', (SELECT count(*) FROM assistente_observacoes WHERE person_id = p_absorver),
...
BEGIN
  ...
  -- #305: observações do mentor para a assistente. Sem unicidade — todas passam para o mantido.
  WITH m AS (UPDATE assistente_observacoes SET person_id = p_manter WHERE person_id = p_absorver RETURNING id)
    SELECT coalesce(jsonb_agg(id), '[]'::jsonb) INTO v_ids FROM m;
  v_movidos := v_movidos || jsonb_build_object('assistente_observacoes', v_ids);
